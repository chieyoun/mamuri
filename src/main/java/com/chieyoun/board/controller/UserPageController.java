...
import com.chieyoun.board.service.*;
...
//    private final SaleService saleService;
    ...
    public String userCartPage(@PathVariable("id") Integer id, Model model, @AuthenticationPrincipal PrincipalDetails principalDetails) {
        ...
        if (principalDetails.getUser().getId() == id) {
            ...
            JoiningClass userCart = user.getCart();

            // 장바구니에 들어있는 아이템 모두 가져오기
            List<ClsItem> cartItemList = cartService.allUserCartView(userCart);

            // 장바구니에 들어있는 상품들의 총 가격
            int totalPrice = 0;
            for (ClsItem cartitem : cartItemList) {
    ...
    public String deleteCartItem(@PathVariable("id") Integer id, @PathVariable("cartItemId") Integer itemId, Model model, @AuthenticationPrincipal PrincipalDetails principalDetails) {
        ...
        if (principalDetails.getUser().getId() == id) {
            // itemId로 장바구니 상품 찾기
            ClsItem cartItem = cartService.findCartItemById(itemId);

            // 해당 유저의 카트 찾기
            JoiningClass userCart = cartService.findUserCart(id);
...
//
//            // 유저 카트 찾기
//            JoiningClass userCart = cartService.findUserCart(user.getId());
//
//            // 유저 카트 안에 있는 상품들
//            List<ClsItem> userCartItems = cartService.allUserCartView(userCart);
//
//            // 최종 결제 금액
//            int totalPrice = 0;
//            for (ClsItem cartItem : userCartItems) {
