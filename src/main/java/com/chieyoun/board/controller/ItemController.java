...
import com.chieyoun.board.service.ClsService;
...
public class ItemController {
    ...
    public String ItemView(Model model, @PathVariable("itemId") Integer id, @AuthenticationPrincipal PrincipalDetails principalDetails) {
        if(principalDetails.getUser().getRole().equals("ROLE_SELLER")) {
            ...
            int cartCount = 0;
            JoiningClass userCart = cartService.findUserCart(loginUser.getId());
            List<ClsItem> cartItems = cartService.allUserCartView(userCart);
